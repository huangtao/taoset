...
def gen_png_from_plist(plist_filename, png_filename):
    ...
    for k,v in plist_dict['frames'].items():
        ...
        if outfile.find('.png') == -1:
            outfile = outfile + '.png'
        if rect_on_big.size[0] != result_box[2] or rect_on_big.size[1] != result_box[3]:
            if rect_on_big.size[0] == result_box[3]:
                result_box = (0, 0, width, height)
        if rect_on_big.size[0] != result_box[2] or rect_on_big.size[1] != result_box[3]:
            print outfile, rect_on_big.size[0],rect_on_big.size[1],result_box[2],result_box[3]
        else:
