    ...
    while flag_exit is False:
        if is_stop == False:
            ...
            if hwnd == 0:
                ...
                img = PIL.Image.frombuffer('RGB', (bmpinfo['bmWidth'], bmpinfo['bmHeight']), bmpstr, 'raw', 'BGRX', 0, 1)
                if debug:
                ...
                if is_full:
                    ...
                    bmpinfo = saveBitmap.GetInfo()
                    bmpstr = saveBitmap.GetBitmapBits(True)
                    imgPop = PIL.Image.frombuffer('RGB', (bmpinfo['bmWidth'], bmpinfo['bmHeight']), bmpstr, 'raw', 'BGRX', 0, 1)
                    if debug:
                    ...
                    if (px1_r == 109 and px1_g == 196 and px1_b == 252 and
                        ...
                        # 检测到弹出对话框，点击关闭它
                        # 知道了、分享加倍
                        do_click(hwnd, 240, 560)
                        time.sleep(10)
                    # 自动加下电力
                    # time.sleep(2)
                    # do_click(hwnd, 240, 50)
